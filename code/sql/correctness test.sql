...
insert into Location(province, city, district) values('上海', '上海', '闵行'); /* 地址1 */
insert into Location(province, city, district) values('上海', '上海', '徐汇'); /* 地址2 */
...
insert into borrowingitems values(1, 2); /* 加入借书车 */

/* 确定借书 */
delete from borrowingitems where R_ID = 1 and ID = 2; 
insert into borrowinghistory(OrderStatus, Location, ID, R_ID) values(0, '上海交通大学徐汇校区', 2, 1);
update bookReleases set `Status` = 1 where R_ID = 1;
update users set points = points-10 where ID = 2;

/* 图书确认送达，未评价 */
update borrowinghistory set OrderStatus = 1 where H_ID = 1;

/* 借阅评价 */
insert into borrowcomments(H_ID, BorrowComment, Score) values(1, '符合描述', 10);
update borrowinghistory set OrderStatus = 2 where H_ID = 1;
